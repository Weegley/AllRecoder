...
#ENCODER_PARAMS_AV1 = "-c:V:0 libsvtav1 -svtav1-params keyint=10s:scd=1:enable-qm=1:tune=0" #up to 3-4 times faster than x265, blu blurry picture.
...
def set_params_from_cmd():
    ...
    parser = argparse.ArgumentParser(prog='AllRecoder',
                    description='Recode all videos in folder and subfolders selecting best codec', epilog="", add_help=False, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def get_video_info(input_file):
    ...
    if dur.hour==0:
        ...
        #duration=f"{dur.minute:02}m {dur.second:02}.{str(dur.microsecond)[:1]}s"
    else:
        duration=datetime.strftime(dur,"%H:%M:%S.{}".format(str(dur.microsecond)[:2]))
        #duration=f"{dur.hour:02}h {dur.minute:02}m {dur.second:02}.{str(dur.microsecond)[:1]}s"
